...
sketch \change{a desired} pattern onto the canvas. %Overall, bottom-up querying via drag-and-drop was more intuitive and more commonly used than top-down querying methods, such as sketching or input equations.
The \change{reason why most} participants
did not find \change{direct sketching useful} was that
...
\change{suggests} that past work has primarily focused
...
led to context creation 60\% of the time
...
reflects how astronomers often needed to iteratively
...
often constructed a central workflow
around a main sensemaking process
and interleaved variations with the two other processes
as they iterated on the analytic task}. For example, material scientists focused
...
was tightly coupled with \change{the problem characteristics associated with each domain, as illustrated in Appendix Table~\ref{science_task}.} For example, without an initial query in mind,
