...
that allow users to specify \change{and search for }desired visual patterns
...
a sketching canvas
where users can draw a \change{visual} pattern of interest,
...
\change{appears} to be a promising solution
...
analysts and how they are actually used in practice}. \change{Furthermore, working with participants from multiple domains (an uncommon practice for visualization design studies) enabled us to compare the differences and commonalities across different domains, thereby identifying general VQS challenges and requirements for supporting common analytical goals.} Our contributions include:
\begin{denselist}
...
\agp{Expand on evaluation study findings. Argue for why we studied multiple domains (which is not typical) and the challenges in doing so.}\dor{Added.}
