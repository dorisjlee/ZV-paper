...
\par 
 ...
 we first identified challenges
in existing data analysis workflows in these domains
that could be potentially 
addressed by a VQS. 
Building on top of an existing, 
open-source VQS, \zv~\cite{Siddiqui2017,Siddiqui2017VLDB}, 
we collaborated closely with our participants
to gather feedback and iterate on VQS feature designs. 
We organized these features into a taxonomy of VQS functionalities,
involving three sensemaking processes inspired by X et al.,
\agp{}---top-down, bottom-up,
and context-creation.\agp{A one line explanation of this would
be valuable.}
We find that prior VQSs have focused
largely on top-down processes,
while largely ignoring the other two processes
that are crucial for the needs of all three domains.
...
two processes as they iterate on an analytic task\agp{mirroring findings in X et al?}.
...
\item evaluation study findings on how VQSs are used in practice, leading to the construction of a novel sensemaking model for VQSs. %including the ineffectiveness of sketching and the ---- workflow  
\end{denselist}
