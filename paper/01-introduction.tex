...
\tvcg{Based on our early interactions with data analysts, 
we started to build a VQS~\cite{Siddiqui2017VLDB,Siddiqui2017} that, similar to existing VQSs~\cite{wattenberg2001sketching}, allowed users to search for desired trends via drawing on a canvas. This early system served as a functional prototype for us to engage with the participants, understand how they envision themselves using a VQS, and gather feedback on feature designs that could make the VQS more useful. The features we developed address challenges shared across multiple use-cases, ranging from additional querying modalities, to features that support a more integrated workflow, to improving the interpretability of the system output, not all of which were supported by prior VQSs in the literature. Our collaborative design experience culminated in a full-fledged VQS, \zv, capable of provoking rapid iterations of different data operations and hypotheses that enable insight-discovery. 
%focused on the systems and scalability aspects of the VQSs and
}\agp{not clear that you started with ZV, then extended it}\dor{Fixed.}
...
%\tvcg{We interacted with the participants on a regular basis to gather feedback on our VQS and understand how they envision themselves using it. The features we developed address challenges shared across multiple use-cases, ranging from additional querying modalities, to features that support a more integrated workflow, to improving the interpretability of the system output, not all of which were supported by prior VQSs in the literature.}
...
\\ To evaluate our final system, we conducted a user study with nine scientists (including those who had participated in the design), all of whom had a vested interest in using a VQS to address their research questions on their datasets. In a 1.5-hour user study, our scientist participants were able to gain novel scientific insights, such as \tvcg{identifying a star with a transit pattern that was known to harbor a Jupiter-sized planet,} finding characteristic gene expression profiles that confirmed the results of a related publication, and learning that the dip in an astronomical light curve is caused by saturated imaging equipment overlooked by the existing error-flagging pipeline. 
