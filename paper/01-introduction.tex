...
they want to visualize. \tvcg{From our early discussions with analysts from twelve different application areas, we learned that analysts often need to search for their desired pattern amongst large numbers of visualizations.} For example, when trying to find celestial objects
corresponding to supernovae, which have a specific pattern
of brightness over time, scientists
need to individually \tvcg{inspect the visualizations for} \sout{visualize the brightness of} 
...
\cut{One such VQS \nonannon{developed by us} is \zv~\cite{Siddiqui2017VLDB}, 
which supports multiple modes of specification, including a sketching canvas
where users can draw a pattern of interest, or drag and drop an existing visualization onto
the same canvas, with the system finding visualizations that are similar to the queried pattern. 
Similarly, Google Correlate~\cite{mohebbi2011google} 
and QuerySketch~\cite{wattenberg2001sketching} allow users to draw a trend of interest,
with the system automating the search to find matching visualizations.}
...
% \kk{As we discussed earlier today, we should define a VQS. Here we hint at what it is by describing other VQSs, but this approach leaves the term somewhat ambiguous}
% \kk{Either here or in the related work, make clear how ZV differentiates itself from these other VQSs.}


\cut{Overall, VQSs appear to be an ideal fit for the challenge of visualization exploration
described earlier---in that it automates the painful manual exploration
of visualizations to find desired patterns.}
\tvcg{Not surprisingly, when asked about the potential viability of VQSs in aiding scientific data analysis,  many analysts indicated that VQSs could be useful in mitigating the challenge of visualization exploration described earlier---since it automates the painful manual exploration of visualizations to find desired patterns.}
...
\tvcg{Based on the our early interactions with these analysts, 
we first built a basic VQS \zv~\cite{Siddiqui2017VLDB,Siddiqui2017}, 
that, similar to existing VQSs~\cite{wattenberg2001sketching}, allowed users to search for desired trends via drawing on a canvas\cut{, while using an optimized execution engine for scaling to large collections of visualizations with interactive response times}\dor{this part is probably not necessary and also is not something existing VQS has}. Building on top of our functional prototype, we worked closely with the scientists and developed 20 major features through multiple iterations.} The features we developed address challenges shared across multiple use-cases, ranging from additional querying modalities, to features that support a more integrated workflow, to improving the interpretability of the system output, not all of which were supported by prior VQSs in the literature. We interacted with the participants on a regular basis to gather feedback on our VQS and understand how they envision themselves using it.
...
\emph{RQ3: How does VQSs accelerate scientific insights?}\\
...
\vspace{-6pt}\tvcg{\caption{Descriptions of the three scientific use cases discussed in this paper.}}
\label{example}
\vspace{-10pt}
\end{figure*}