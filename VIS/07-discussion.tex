%!TEX root = main.tex
 ...
 than top-down processes for generating a pattern query}.} Within top-down processes,
 ...
 \npar \fchange{\stitle{Bottom-up }pattern queries can come from either
 the ranked list of results,
 recommendations, or by selecting a
 ...
 \begin{figure*}[ht!]
   ...
   \vspace*{-15pt}
 \end{figure*}
 \npar \fchange{\stitle{Context creation} in VQSs enables users to change the ``lens''
 ...
 only the differentially expressed genes?} \techreport{\texttt{DIFFEXP=1} }(G2) or \textit{Can I find more supernovae candidates if I query only on objects that are bright and classified as a star?} \techreport{\texttt{flux\textgreater10 AND CLASS\_STAR=1} }(A1). Three participants had also used filtering as a way to query with known individual objects of interest, as shown in \fchange{the \bartext{object of interest} bar of} Figure~\ref{fig:origins_of_sketch}. For example, G2 set the filter as gene=9687 and explained that since ``\textit{this gene is regulated by the estrogen receptor, when we search for other genes that resemble this gene, we can find other genes that are potentially affected by the same factors.}''
