...
% Likewise, Figure~\ref{query_modification} (right) illustrates how A3 first picked out a regular pattern (suspected starspot), then modified it slightly so that the pattern looks more \rchange{``irregular''} (to find pulsating stars).
 ...
 move between different sensemaking processes\rchange{\footnote{We chose to break down these results by the domain, rather than on an individual basis, because we found that the analytical patterns are very similar within the domains (possibily due to how similiar the analytical inquiries and datasets were within the domains).}}. For example, in material science,
