...
% Likewise, Figure~\ref{query_modification} (right) illustrates how A3 first picked out a regular pattern (suspected starspot), then modified it slightly so that the pattern looks more \rchange{``irregular''} (to find pulsating stars).
 ...
 we compute the aggregate state transition probabilities (edge weight labels in Figure~\ref{fig:transition}) to characterize how participants from each domain move between different sensemaking processes\rchange{\footnote{We chose to break down these results by the domain, rather than on an individual basis, since we found that the analytical patterns are very similar within the domains (possibily due to how similiar the analytical inquiries and datasets were within the domains).}}. For example, in material science, \rchange{60\% of events that started with }bottom-up exploration \rchange{lead} to context creation and to top-down pattern search the rest of the time.
