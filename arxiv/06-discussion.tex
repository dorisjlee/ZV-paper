...
\par Both A1 and A2 expressed that \fchange{context creation through} interactive filtering enabled them to test conditions and tune values that they would not have otherwise modified, effectively lowering the barrier between the iterative hypothesize-then-compare cycle during sensemaking.
...
is tightly coupled with \change{the problem characteristics associated with each domain, as illustrated in Appendix Table~\ref{science_task}.} For example, without an initial query in mind,
